...
- [x] **API-01**: User can POST /generate with `{ prompt, targetPlatforms }` and receive `{ requestId, results }` with platform-keyed outputs
...
- [x] **PIPE-01**: System generates a canonical MusicConcept (title, genre, mood, BPM, instruments, description) from the user's prompt via the LLM provider
...
- [x] **PIPE-03**: Platform processors run in parallel using `Promise.allSettled` so one failure does not block other platforms
- [x] **PIPE-04**: If a platform processor fails, the system reconstructs output from MusicConcept and includes `"fallback": true` in that platform's response; other platforms return normally
- [x] **PIPE-05**: `GenerationService` orchestrates: receive request → generate MusicConcept → resolve processors → run in parallel → persist → return response

### Platform Processors

- [x] **PROC-01**: `SpotifyProcessor` transforms MusicConcept into `{ title, genre, mood, bpm, instruments, description }`
- [x] **PROC-02**: `TikTokProcessor` transforms MusicConcept into `{ hook, hashtags }` with exactly 3 hashtags drawn from established genre hashtag patterns
- [x] **PROC-03**: `YouTubeProcessor` transforms MusicConcept into `{ title, description, tags }` with SEO-optimized content
- [x] **PROC-04**: All processors implement the `PlatformProcessor` interface: `readonly platform: string; generate(concept: MusicConcept): Promise<PlatformResult>`
- [x] **PROC-05**: `PlatformRegistry` resolves processors by platform name and provides the processor collection; adding a new platform requires only registering a new processor — no changes to existing code

### Caching

- [x] **CACHE-01**: Successful generations are cached in Redis using `hash(prompt + sortedPlatforms)` as key
- [x] **CACHE-02**: Identical requests (same prompt + same platforms in any order) return cached result without calling the LLM
- [x] **CACHE-03**: Cache misses trigger normal LLM generation; cache hits skip LLM and return stored result
...
- [x] **PERSIST-01**: Each generation request is stored in `generation_requests` table with `id`, `prompt`, `created_at`
- [x] **PERSIST-02**: Each platform result is stored as a separate row in `generation_results` table with `id`, `request_id`, `platform`, `payload_json`, `created_at`
