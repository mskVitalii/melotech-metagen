...
## AI Usage Documentation (DOC-01)

### AI Tools Used

- **GitHub Copilot** (Claude Sonnet 4.6) — primary coding assistant for architecture, all backend and frontend code
- **GSD workflow** (`@opengsd/gsd-core`) — structured project management, phase planning, and context gathering

### What Was AI-Generated

**Architecture & Planning**

- Project roadmap and phase breakdown (ROADMAP.md, REQUIREMENTS.md)
- Technology stack selection and version research (STACK.md in `.planning/`)
- Phase context gathering and implementation decisions (`.planning/phases/`)

**Backend (`backend/src/`)**

- NestJS module structure (AppModule, GenerationModule, LLMModule, CacheModule, ThrottlerModule)
- Prisma schema design (two-table model: `generation_requests` + `generation_results`)
- `LLMProvider` abstraction + `OpenAIProvider` with structured output via `zodResponseFormat`
- `MusicConcept` Zod schema
- `PlatformProcessor` interface + Spotify/TikTok/YouTube processors
- `PlatformRegistry` multi-provider pattern
- `GenerationService` orchestration (cache → LLM → fan-out → fallback → persist → cache write)
- `PersistenceService` with `$transaction` + `findHistory()` (single-query pagination, no N+1)
- `HistoryController` (`GET /history` with platform filter and pagination)
- `ThrottlerExceptionFilter` for exact 429 + Retry-After response
- Railway deployment config

**Frontend (`frontend/`)**

- Next.js App Router structure (layout, providers, pages)
- TanStack Query v5 client setup (`useQuery`, `useMutation`)
- Generate page: prompt form, platform multi-select, side-by-side comparison view
- History page: paginated list with platform filter
- History detail page: full platform output view
- Shared API client (`lib/api.ts`) and type definitions (`lib/types.ts`)
- Tailwind v4 CSS-first styling

### What Was Manually Reviewed

- Platform processor output shapes verified against SPEC.md examples
- Prisma schema field names and column mappings
- Multi-provider NestJS pattern (double registration: bare class + alias)
- Rate limiting with real client IP via `trust proxy` + `X-Forwarded-For`
- ESM `.js` extension requirements for all local imports in NestJS

### AI's Role in Architecture

- **LLMProvider abstraction**: decouples OpenAI SDK from all platform processors — adding a new platform requires zero changes to existing code
- **Canonical MusicConcept**: single Zod schema used by both the LLM structured output call and all downstream processors
- **Two-phase caching**: `hash(prompt + sorted platforms)` makes identical requests deterministic regardless of platform order
- **Single-query history**: `include: { results: true }` + `Promise.all([findMany, count])` satisfies the no-N+1 PERSIST-03 requirement
